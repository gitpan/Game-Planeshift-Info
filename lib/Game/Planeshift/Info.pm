...
# 			print "===> DEBUG <===\n\n$html\n\n===> END DEBUG <===\n\n";
			foreach (split(/<\/TR>/,$html))
			{
				...
				if($1)
				{
					...
					$data->{most_active_players}->{$name} = $hours if(defined($name) && defined($hours)) ;
				}
			}
		}
...
# 			print "===> DEBUG <===\n\n$html\n\n===> END DEBUG <===\n\n";
			foreach (split(/<\/TR>/,$html))
			{
				...
				if($1)
				{
					...
					$data->{best_player_average}->{$name} = {day_active => $day_active,hours_per_day => $hours_per_day} if(defined($name) && defined($day_active) && defined($hours_per_day));
				}
			}
		}
...
# 			print "(adv) \$k is now at $k/$#raw_data\n";
			foreach (split(/<\/TR>/,$html))
			{
				...
				if($1)
				{
					...
					$data->{best_advisors}->{$name} = $advisor_points  if(defined($name) && defined($advisor_points));
				}
			}
		}
...
# 			print "(duel) \$k is now at $k/$#raw_data\n";
			foreach (split(/<\/TR>/,$html))
			{
				...
				if($1)
				{
					...
					$data->{dueling_champs}->{$name} = $duel_points  if(defined($name) && defined($duel_points));
				}
			}
		}
...
# 			print "(str) \$k is now at $k/$#raw_data\n";
			foreach (split(/<\/TR>/,$html))
			{
				...
				if($1)
				{
					...
					$data->{stronger_players}->{$name} = $str  if(defined($name) && defined($str));
				}
			}
		}
...
# 			print "(int) \$k is now at $k/$#raw_data\n";
			foreach (split(/<\/TR>/,$html))
			{
				...
				if($1)
				{
					...
					$data->{cleverer_players}->{$name} = $int  if(defined($name) && defined($int));
				}
			}
		}
	}
...
C<bug-Game-Planeshift-Info at rt.cpan.org>, or through the web interface at
L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=Game-Planeshift-Info>.
...
L<http://annocpan.org/dist/Game-Planeshift-Info>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/Game-Planeshift-Info>

=item * RT: CPAN's request tracker

L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=Game-Planeshift-Info>

=item * Search CPAN

L<http://search.cpan.org/dist/Game-Planeshift-Info>
